...
import { createAndVerifyReader, createAndVerifyReaderFromBuffer } from './util/shapeTestUtils';
...
describe('ShapeReader', () => {
  ...
  describe('extent (fromArrayBuffer)', () => {
    it('should produce the same result via raw ArrayBuffer input', async () => {
      const reader = await createAndVerifyReaderFromBuffer('polygon.shp', 'polygon.shx', ShapeType.Polygon, 3);
      expect(reader.extent.xMin).toBeCloseTo(-263.58181430356285, 7);
      expect(reader.extent.xMax).toBeCloseTo(100, 7);
      expect(reader.extent.yMin).toBeCloseTo(-141.77167490179193, 7);
      expect(reader.extent.yMax).toBeCloseTo(60, 7);
